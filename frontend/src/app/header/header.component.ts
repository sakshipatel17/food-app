...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  logout() {
    localStorage.clear();
    this.router.navigate(['/home']); // or /login if you have it
  }
  ...
  ngOnInit(): void {
    ...
    this.api.cartCount.subscribe((data: any) => {
      ...
      if (this.cartCount > prevCount) {
        this.cartBadgeAnimation = true;
        setTimeout(() => {
          this.cartBadgeAnimation = false;
        }, 300);
      }
    });

    // Check if logo animation has already played this session
